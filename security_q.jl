...
mutable struct Customer
    id::Int64
    arrival_time::Float64       # time when the customer arrives at the factory type float
    server::Union{Missing,Int64}# ID of server where the customer is served type int
    start_service_time::Float64 # time when the customer starts service type float
    completion_time::Float64    # time when the customer is complete type float
end
...
struct Arrival <: Event # customer arrives
    id::Int64         # give a unique event id
    time::Float64     # and the time of the event 
end
